...
【蒙版】快捷键 (右侧切到蒙版后):\n\
  ...
  {m}S 保存工程 (修图里也是保存, 不是应用). 分块改了这条的上下沿后, 拼合和导出改用当前裁切, 修图文件留着, 列表标「脱离」; 改回原范围后自动重新套用. 蒙版里再拖边不取消修图.\n\
  修图改变条带高度时, 完全在这块下方的遮盖跟着平移; 压在这块上的遮盖留在原位置.\n\
...
【视频】快捷键 (右侧切到视频后, 先在轨道/预览区点一下获得焦点):\n\
  ...
  N 在播放头插入下一张组合 (按素材池顺序自动顺延) | W 刷入下一张 (切页并附 1 秒左→右转场)\n\
  I 标记淡入 | O 标记淡出\n\
  Delete / Backspace 删除当前选中的视频片段/淡入淡出/音频片段\n\
  {m}Z/Y 撤重 (时间轴操作)\n\
  鼠标: 拖动片段两端裁剪, 拖动片段整体移动; 淡入淡出轨道可直接拖选一段生成区间,\n\
  {m}点击可多选淡入淡出; 右键「保持背景为底色」只淡乐谱、不淡到黑 (多选则一起应用),\n\
  开启后色块变浅并标「·底」; 刷入转场钉在翻页边界, 只能拖右边缘改时长或拖页面边界一起移动,\n\
  不能叠在淡入淡出上; 视频/淡入淡出/音频边界彼此靠近时自动吸附; 时间轴总长对齐最短的非空音/视频轨\n\
...
\n\
操作步骤:\n\
1. 打开/拖入图片或 PDF → 多标签页; 页图写入会话临时目录, 内存只留按视口缩小的预览代理 (默认当前页±4, 高清页自动收到 ±2/±1; 输出组合/蒙版页签仍列出全部); 放大超过预览密度时再按窗口裁切原图.\n\
...
9. 「蒙版」编辑当前组合的竖向拼合图; 组合标签与分块一致为「排序号. 来源号」\n\
   ...
   未选中蒙版时: 拖块本体上下移动, Shift+拖左右移 (可超出原块左右, 蒙版界面仍画出并在原边界示意; 底色/视频/导出按原块左右截掉),
   拖上下边或左右边裁切/扩展 (上下先消耗块间空白, 到页顶/页底即停; 左右向内裁掉的空位用谱纸色填, 向外超出仍按原块左右截掉; 拖动中用分块贴图跟手).\n\
...
11. 「底色」右侧面板把底色作为可撤销的底层叠加到各输出组合 (不卡界面),\n\
    ...
    工程里页面按谱面宽定高, 谱面再高也只缩小装进去; 批量在装不下高度时改为按高度, 页面变宽.\n\
