...
\n\
【蒙版】快捷键 (右侧切到蒙版后):\n\
   初始为选择态 (不激活框选); 未选中任何蒙版时可直接拖动/拉伸「组合分块」 (拖上下移; Shift+拖左右移, 可超出页面; 拖动中贴图跟手)\n\
...
9. 「蒙版」编辑当前组合的竖向拼合图; 组合标签与分块一致为「排序号. 来源号」\n\
   ...
   未选中蒙版时: 拖块本体上下移动, Shift+拖左右移 (可超出页面, 蒙版界面仍画出; 视频/导出按页宽截掉),
   拖上下边裁切/扩展 (先消耗块间空白; 到页顶/页底即停; 拖动中用分块贴图跟手).\n\
