...
【分块】快捷键:\n\
  ...
  D 识别本页 | A 识别全部页\n\
  N 添加新块 | S 分割块 | M 合并组合 | Ctrl+M 一键两两合并 | U 拆开组合 | G 共享脚注 | Delete 删除\n\
...
【视频】快捷键 (右侧切到视频后, 先在轨道/预览区点一下获得焦点):\n\
  ...
  Ctrl 点击可多选淡入淡出; 右键「保持背景为底色」只淡乐谱、不淡到黑 (多选则一起应用),\n\
...
7. Ctrl 多选可跨页, 「合并组合」(M); 脚注可用「共享脚注」让同一块出现在多组导出中.\n\
   Ctrl+M 一键两两合并: 把本页尚未组合的块按上下顺序两两配对; 若剩一块, 则与下一页\n\
...
12. 「视频」页: 上方预览窗 (悬浮显示可拖动的进度条), 下方视频/淡入淡出/音频三条轨道;\n\
    ...
    淡入淡出可 Ctrl 多选, 右键「保持背景为底色」则只淡乐谱内容、不淡到黑.\n\
