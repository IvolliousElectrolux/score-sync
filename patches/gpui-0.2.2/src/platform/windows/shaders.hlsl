...
float4 polychrome_sprite_fragment(PolychromeSpriteFragmentInput input): SV_Target {
    ...
    float2x2 rs = sprite.transformation.rotation_scale;
    bool rotated = abs(rs._m00 - 1.0) > 0.001 || abs(rs._m11 - 1.0) > 0.001
        || abs(rs._m01) > 0.001 || abs(rs._m10) > 0.001;
    float distance = rotated ? -1.0 : quad_sdf(input.position.xy, sprite.bounds, sprite.corner_radii);
